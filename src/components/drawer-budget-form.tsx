...
export function DrawerBudgetForm(props: Props) {
  const [enrolling, { Form }] = createServerAction$(async (form: FormData, { request }) => {
    ...
    try {
      const record = await pb.collection('budget_groups').create({
        name: form.get('name'),
        user_id: userId,
      });
  ...
  return (
    <Drawer isOpen={props.isOpen} onToggle={props.onToggle} class="w-full max-w-xl">
      ...
      <DrawerContent>
        <DrawerHeader>
          <DrawerTitle>Add a new budget group</DrawerTitle>
          <DrawerDescription>Fill the form below to add a new budget group.</DrawerDescription>
        </DrawerHeader>
        <DrawerBody>
          <Show
            when={!result()}
            fallback={
              <div class="mt-10 flex flex-col items-center justify-center rounded-md border-2 border-black p-6">
                ...
                <p class="text-xl font-semibold text-gray-900">Budget group added</p>

                <p class="mt-5 text-center text-lg text-gray-900">
                  Your budget group has been added successfully. You can close this form now.
                </p>
                <div class="mt-6">
                  <Button
                    fw={'semibold'}
